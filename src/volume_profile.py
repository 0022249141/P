...
class VolumeProfileEngine:
    ...
    Builds a dynamic volume profile over a lookback window and identifies VPOC.
    """
    def __init__(self, market_engine, lookback_candles=96):  # 96 candles of 15min = 24h
    ...
    def _calculate_vpoc(self, num_bins=50):
        """Compute rolling VPOC for each candle based on previous N candles."""
        for i in range(self.lookback, len(self.df)):
            window = self.df.iloc[i - self.lookback : i]
            price_range = np.linspace(window['low'].min(), window['high'].max(), num_bins)
            ...
            for _, row in window.iterrows():
                ...
                for j in range(num_bins - 1):
                    ...
                    if overlap > 0:
                        volume_profile[j] += vol * (overlap / (high - low))

            if volume_profile.sum() > 0:
                max_bin = np.argmax(volume_profile)
                vpoc_price = (price_range[max_bin] + price_range[max_bin+1]) / 2
                self.df.loc[self.df.index[i], 'vpoc'] = vpoc_price

    def get_vpoc(self, idx: int) -> float:
        return self.df['vpoc'].iloc[idx] if not pd.isna(self.df['vpoc'].iloc[idx]) else None

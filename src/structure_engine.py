...
class StructuralEngine:
    """
    Layer 2 — Structural analysis with swing strength (repaint-free)
    """
    ...
    def detect_swings(self, window: int = 5, min_strength: float = 0.8):
        """
        Detect swing highs and lows using relative strength (ATR-based).
        No repaint: uses a centered window.
        """
        ...
        for i in range(window, len(self.df) - window):
            ...
            is_high = high == np.max(self.df['high'].iloc[i - window : i + window + 1])
            is_low  = low  == np.min(self.df['low'].iloc[i - window : i + window + 1])
    ...
    def is_bos(self, idx: int, direction: str) -> bool:
        """
        Check Break of Structure (BOS) with penetration buffer.
        direction: 'bullish' (break above last swing high) or 'bearish'.
        """

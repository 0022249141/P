...
class MarketDataEngine:
    ...
    """
    def __init__(self, df: pd.DataFrame, atr_period: int = 14, regime_lookback: int = 100):
        self.df = df.copy()
        ...
        self.df['atr_low_percentile'] = self.df['ATR14'].rolling(regime_lookback).apply(
            lambda x: np.percentile(x, 25) if len(x) == regime_lookback else np.nan, raw=True
        )
        self.df['atr_high_percentile'] = self.df['ATR14'].rolling(regime_lookback).apply(
            lambda x: np.percentile(x, 75) if len(x) == regime_lookback else np.nan, raw=True
        )
    ...
    @classmethod
    def from_custom_csv(cls, filepath: str, atr_period=14, regime_lookback=100):
        """
        ...
        return cls(df, atr_period=atr_period, regime_lookback=regime_lookback)
